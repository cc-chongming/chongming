...
- 2026-08-27：段 3（用户批注“上下文侦察也固定高度内部滚动”）：`.flow-content > .scout-conclusion-panel` 加入定高内滚体系，收集结论卡与运行流面板分享剩余高度，侦察阶段中间列不再整列滚动。
- 2026-08-27：追加段 2（用户批注“滚动条丑”）：`.review-flow-page` 全域细滚动条——轨道透明、滑块 #d6d3d1 圆角、悬停加深、Firefox thin；覆盖定高内滚区块、对话流、议题 Tab 横向滚动等全部容器。
- 2026-08-27：追加段 4（用户批注“辩论对话流和对抗看板分开两块，分别内部固定滚动”）：`flow-debate-scroll` 单滚动容器拆为 `flow-debate-split`（弹性列）内两块——`flow-debate-board`（法庭+中立方）与 `flow-debate-dialogue` 各自 flex 填充、独立内滚，块间距走容器 gap。

...
> **2026-08-10 体验修订**: AIREVIEW-PLAN-023 覆盖本计划的用户可见对话边界。新版 /live 不展示模型 reasoning；工具调用默认折叠并可按需查看入参/出参；所有 Agent 使用统一公开 AI 对话组件；真实流式必须来自 Provider 与后端 delta，不得由前端动画伪造。本计划中“展示 thinking”和“原始工具块作为主流程内容”的旧描述仅保留为历史实施记录，不再作为当前验收依据。

...
| 2026-08-18 | 修复线上仓库评审 Context Scout 必然降级：`buildSharedProjectContext` 以 `requirement.repositoryPath()` 作为共享概览身份，而远程绑定需求该字段为 null，`Map.of` 直接 NPE 导致 Scout 在构建期静默降级；改为取 `snapshot.repositoryId()`。降级路径补 `context_scout_degraded` WARN 根因日志；新增真实 Harness 构建回归、远程绑定概览回归与活体 workspace 诊断复现测试。 |

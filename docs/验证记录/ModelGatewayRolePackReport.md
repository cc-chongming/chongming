...
| 验证项 | 覆盖测试 | 结果 |
|---|---|---|
| Profile 配置绑定 | `ModelProfilesPropertiesTests`、`ReviewPropertiesTests` | 四类逻辑 Profile 可绑定；默认关闭时不要求凭证，启用时必须提供 Base URL 和环境变量凭证 |
...
- 真实商业模型冒烟：部署方配置受控 Base URL、模型 ID 与环境变量凭证后，每个 Profile 执行一次；不得记录密钥或完整 Prompt。

...
**规则（2026-08-27 用户最终确认：一切支持都显示“支持”，立场优先）**：
- SUPPORT（任意 severity）→ 类别“支持”，摘要计入“支持”；
- 其余立场按 severity：P0 阻断、P1 高风险、P2 改进（建议）、P3 提示。
- （首版曾按字面示例实现“仅 P2/P3 支持转支持”，用户批注确认后切换为全立场口径，即风险节预留方案。）
...
- **风险（已兑现）**：SUPPORT·P0/P1 首版仍显示阻断/高风险 → 用户批注确认“一切支持都显示支持”，2026-08-27 切换为立场优先口径（`claimCategoryLabel` + `claimOverview` 分桶同步修订）。
...
- 2026-08-27：口径修订。用户批注“一切支持都显示支持”：`claimCategoryLabel` 立场优先（SUPPORT→支持，不看严重度），`claimOverview` 支持桶=全部 SUPPORT、阻断/高风险/改进/提示只计非支持类；测试同步（PM 实例变为“发现 4 项：0 项阻断、1 项高风险、0 项改进建议、3 项支持”），148 全绿，构建产物 index-94oeTgpX.js 已同步运行服务。

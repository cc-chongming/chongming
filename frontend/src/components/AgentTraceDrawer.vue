...
<template>
    <aside v-if="role" class="evidence-drawer agent-trace-drawer" :class="{ 'agent-trace-embedded': embedded }" aria-label="角色执行过程">
        ...
        <ol class="ag-ui-messages"><li v-for="(event, index) in visible" :key="`${event.runId}-${index}`" class="ag-ui-message"><header><strong>{{ label(event) }}</strong><span>{{ event.runId }}</span></header><p>{{ content(event) }}</p></li><li v-if="!visible.length" class="empty-note">该角色尚未产生可展示的 AG-UI 事件。</li></ol>
    </aside>
</template>
